...
class SimpleTradingBot:
    ...
    def _run_integrated_analysis(self, account_balance: float):
        ...
        for idx, symbol in enumerate(self.config["TRADE_PAIRS"], 1):
            ...
            try:
                # 1. 执行博弈论分析（市场微观结构）
                game_theory_analysis = self.enhanced_analyzer.analyze_market_intent(symbol)
    ...
    def _perform_technical_analysis(self, symbol: str) -> Dict[str, Any]:
        """执行技术分析 - 修复颜色问题"""
        try:
            print_colored(f"📈 执行传统技术分析...", Colors.INFO)

            # 获取历史数据
            df = self.get_market_data_sync(symbol)

            if df.empty:
                print_colored(f"    ⚠️ 无法获取数据，返回默认值", Colors.WARNING)
            ...
            try:
                ...
                print_colored(f"    ⚠️ 指标计算失败: {e}", Colors.WARNING)
                # 使用基础指标
                df = self.calculate_basic_indicators(df)
            ...
            print_colored(f"    • MACD: {'金叉' if macd > macd_signal else '死叉'}", Colors.INFO)
            print_colored(f"    • 布林带位置: {bb_position:.1f}%", Colors.INFO)
            print_colored(f"    • 趋势: {trend_direction}", Colors.INFO)
            print_colored(f"    📊 技术分析总分: {signal_strength:.1f}", Colors.YELLOW)

            return technical_analysis

        except Exception as e:
            self.logger.error(f"技术分析失败: {e}")
            print_colored(f"❌ 技术分析失败: {str(e)}", Colors.ERROR)
            return self.get_default_technical_analysis(symbol)
    ...
    def _integrate_analyses_trend_first(self, game_theory: Dict, technical: Dict, symbol: str) -> Dict[str, Any]:
        ...
        try:
            # 1. 获取技术分析得分
            tech_signal_strength = float(technical.get('signal_strength', 0))
            ...
            if tech_signal_strength * game_score > 0:  # 同向
                final_score *= 1.2
                integrated['signals'].append("技术与博弈共振")
            elif tech_signal_strength * game_score < 0:  # 反向
                final_score *= 0.8
                integrated['risks'].append("信号存在分歧")
            ...
            if final_score >= 2.0:
                integrated['action'] = 'BUY'
                integrated['confidence'] = min(abs(final_score) / 10, 0.9)
                if final_score >= 3.0:
                ...
                integrated['action'] = 'HOLD'
                integrated['reason'] = f"信号不够强烈 (得分: {final_score:.1f})"
    ...
    def calculate_technical_score_v2(technical_analysis: Dict) -> float:
        """
        计算技术分析综合得分 V2
        返回范围：-10 到 10
        """
    ...
    def _calculate_risk_adjusted_params(self, decision: Dict, account_balance: float, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # ========== 智能计算仓位和杠杆 ==========
            position_calc = self._calculate_smart_position_size(account_balance, decision['confidence'])
            ...
            # 根据市场条件调整
            if '可能是诱多' in decision.get('risks', []) or '可能是诱空' in decision.get('risks', []):
                risk_adjustment *= 0.7  # 如果有操纵风险，减少30%
                print_colored(f"    ⚠️ 检测到操纵风险，仓位减少30%", Colors.WARNING)

            if '信号存在分歧' in decision.get('risks', []):
                risk_adjustment *= 0.8  # 信号分歧，减少20%
                print_colored(f"    ⚠️ 信号存在分歧，仓位减少20%", Colors.WARNING)

            # 极端市场条件（如订单簿失衡>50%）
            if 'game_theory_analysis' in decision:
                order_book = decision['game_theory_analysis'].get('order_book', {})
                imbalance = abs(order_book.get('imbalance', 0))
                if imbalance > 0.5:
                    risk_adjustment *= 0.7
                    print_colored(f"    ⚠️ 极端订单簿失衡 ({imbalance:.1%})，仓位减少30%", Colors.WARNING)

            # 应用风险调整
            adjusted_base_amount = base_amount * risk_adjustment
            adjusted_leveraged_value = adjusted_base_amount * leverage

            # 确保调整后仍满足最小要求
            if adjusted_leveraged_value < min_notional:
                print_colored(f"    ⚠️ 风险调整后不满足最小要求，保持最小仓位", Colors.WARNING)
                adjusted_base_amount = min_notional / leverage * 1.05
                adjusted_leveraged_value = adjusted_base_amount * leverage

            # ========== 检查账户余额是否足够 ==========
            if adjusted_base_amount > account_balance * 0.95:
                print_colored(f"    ❌ 所需资金 ${adjusted_base_amount:.2f} > 可用余额", Colors.ERROR)
                return None

            # ========== 计算止损止盈 ==========
            if decision['action'] == 'BUY':
                # 基础止损2%，但根据杠杆调整
                base_stop_pct = 0.02 / (leverage / 10)  # 杠杆越高，止损越紧
                stop_loss = current_price * (1 - base_stop_pct)

                # 支撑位调整
                support_levels = decision['technical_analysis'].get('support_resistance', {}).get('supports', [])
                if support_levels:
                    stop_loss = max(support_levels[-1] * 0.995, stop_loss)

                # 止盈目标
                take_profit = current_price * (1 + base_stop_pct * 2.5)  # 风险回报比2.5

                # 阻力位调整
                resistance_levels = decision['technical_analysis'].get('support_resistance', {}).get('resistances', [])
                if resistance_levels:
                    take_profit = min(resistance_levels[0] * 0.995, take_profit)

            else:  # SELL
                base_stop_pct = 0.02 / (leverage / 10)
                stop_loss = current_price * (1 + base_stop_pct)

                resistance_levels = decision['technical_analysis'].get('support_resistance', {}).get('resistances', [])
                if resistance_levels:
                    stop_loss = min(resistance_levels[0] * 1.005, stop_loss)

                take_profit = current_price * (1 - base_stop_pct * 2.5)

                support_levels = decision['technical_analysis'].get('support_resistance', {}).get('supports', [])
                if support_levels:
                    take_profit = max(support_levels[-1] * 1.005, take_profit)

            # ========== 计算数量和风险 ==========
            quantity = adjusted_leveraged_value / current_price

            # 计算风险金额（基于实际投入的资金）
            risk_pct = abs(current_price - stop_loss) / current_price
            risk_amount = adjusted_base_amount * risk_pct * leverage  # 杠杆放大了风险
            actual_risk_amount = adjusted_base_amount  # 实际最大损失就是投入的本金
            ...
            if risk_reward_ratio < min_risk_reward:
                print_colored(f"    ⚠️ 风险回报比过低 (1:{risk_reward_ratio:.1f} < 1:{min_risk_reward})",
                              Colors.WARNING)
                # 尝试调整止盈以达到最小风险回报比
                if decision['action'] == 'BUY':
                ...
                print_colored(f"    ✅ 调整止盈目标以达到 1:{min_risk_reward}", Colors.INFO)

            # ========== 打印详细信息 ==========
            print_colored(f"\n    💰 交易参数汇总:", Colors.CYAN + Colors.BOLD)
            print_colored(f"      • 账户余额: ${account_balance:.2f}", Colors.INFO)
            print_colored(
                f"      • 基础投入: ${adjusted_base_amount:.2f} ({adjusted_base_amount / account_balance * 100:.1f}%)",
                Colors.INFO)
            print_colored(f"      • 使用杠杆: {leverage}x", Colors.INFO)
            print_colored(f"      • 杠杆后价值: ${adjusted_leveraged_value:.2f}", Colors.INFO)
            print_colored(f"      • 下单数量: {quantity:.6f}", Colors.INFO)
            print_colored(f"      • 入场价格: ${current_price:.4f}", Colors.INFO)
            print_colored(f"      • 止损价格: ${stop_loss:.4f} ({(stop_loss / current_price - 1) * 100:+.2f}%)",
                          Colors.INFO)
            print_colored(f"      • 止盈价格: ${take_profit:.4f} ({(take_profit / current_price - 1) * 100:+.2f}%)",
                          Colors.INFO)
            print_colored(f"      • 最大风险: ${actual_risk_amount:.2f} (本金)", Colors.WARNING)
            print_colored(f"      • 预期盈利: ${adjusted_base_amount * (reward / risk) * leverage:.2f}", Colors.GREEN)
            print_colored(f"      • 风险回报比: 1:{risk_reward_ratio:.1f}", Colors.CYAN)

            return {
                'position_pct': adjusted_base_amount / account_balance,
                'position_value': adjusted_leveraged_value,
                ...
                'expected_profit': adjusted_base_amount * (reward / risk) * leverage,
                'leverage': leverage,
    ...
    def _display_trading_opportunity(self, opportunity: Dict):
        ...
        action_color = Colors.GREEN if opportunity['action'] == 'BUY' else Colors.RED

        print_colored(f"   方向: {action_color}{opportunity['action']}{Colors.RESET}", Colors.INFO)
        print_colored(f"   入场价: ${params['entry_price']:.4f}", Colors.INFO)
        print_colored(
            f"   止损价: ${params['stop_loss']:.4f} ({abs(params['stop_loss'] / params['entry_price'] - 1) * 100:.1f}%)",
            Colors.INFO)
        print_colored(
            f"   止盈价: ${params['take_profit']:.4f} ({abs(params['take_profit'] / params['entry_price'] - 1) * 100:.1f}%)",
            Colors.INFO)
        print_colored(f"   仓位: {params['position_pct'] * 100:.1f}% (${params['position_value']:.2f})", Colors.INFO)
        print_colored(f"   风险金额: ${params['risk_amount']:.2f}", Colors.WARNING)
        print_colored(f"   预期盈利: ${params['expected_profit']:.2f}", Colors.GREEN)
        print_colored(f"   风险回报比: 1:{params['risk_reward_ratio']:.1f}", Colors.CYAN)
    # 辅助方法
    ...
    def _get_symbol_min_notional(self, symbol: str) -> float:
        ...
        try:
            ...
            self.logger.error(f"获取{symbol}最小名义价值失败: {e}")
            # 返回安全的默认值
            return 15.0
